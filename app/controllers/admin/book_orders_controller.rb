class Admin::BookOrdersController < Admin::ApplicationController
  ...
  def index
    @book_orders = Order.where(categoryable_type: 'Book').all.order(created_at: :desc)
    ids = params[:ids].blank? ? [] : params[:ids].split(",")
    @book_orders_xlsx = Order.where(categoryable_type: 'Book').where(id: ids).order(created_at: :desc)
  ...
  def new
    @book_order = Order.where(categoryable_type: 'Book').new
  end
  ...
  def create
    @book_order = Order.where(categoryable_type: 'Book').new(book_order_params)

    respond_to do |format|
      if @book_order.save
        format.html { redirect_to admin_book_orders_path, notice: 'Book order was successfully created.' }
  ...
  def update
    respond_to do |format|
      if ['paid', 'finished'].include?(book_order_params[:status]) && book_order_params[:pick_up_ed].blank?
        @book_order.errors.add(:pick_up_ed)
        format.html { render action: 'edit' }
        format.json { render json: @book_order.errors, status: :unprocessable_entity }
      end

      if @book_order.update(book_order_params)
        format.html { redirect_to admin_book_orders_path, notice: 'Book order was successfully updated.' }
  ...
  private
    ...
    def set_book_order
      @book_order = Order.where(categoryable_type: 'Book').find(params[:id])
    end
